...
\subsection{Label Frequency and Imbalance}
The empirical label distribution of B2V follows a heavily skewed long tail.
Figure~\ref{fig:label-bar} lists the 28 distinct primary labels actually used
by human annotators, sorted by frequency. \emph{price~value} alone covers
42.1\% of the samples, the top-5 labels together account for 79\%, and the
bottom 10 labels each appear in fewer than 1\% of sessions. The same skew is
visible on a log-log scale (Figure~\ref{fig:zipf}): the rank--frequency relation
admits a near-linear fit with slope $-2.25$, indicating a Zipf-like decay even
steeper than the standard $\alpha\!\approx\!1$. As a quantitative summary,
Figure~\ref{fig:lorenz} reports the Lorenz curve of the label distribution
with a Gini coefficient of $0.77$, reflecting strong inequality between head
and tail constructs. Because of this imbalance, we report not only Accuracy
...
\caption{Distribution of the 28 primary labels actually used by annotators.
Bars are colored by the parent TCV dimension. \emph{price~value} alone
accounts for 42.1\% of all samples.}
\label{fig:label-bar}
\end{figure}
...
\subcaption{Zipf--like rank/frequency plot, fit slope $=-2.25$.}
\label{fig:zipf}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{image/figs/fig11_lorenz.pdf}
\subcaption{Lorenz curve of label mass (Gini $=0.77$).}
\label{fig:lorenz}
\end{minipage}
\caption{Two views of B2V's label imbalance.}
\label{fig:imbalance}
\end{figure}

\subsection{TCV Composition and Description Complexity}
...
yields the composition shown in Figure~\ref{fig:tcv-donut}: Functional values
dominate at 76.5\%, followed by Emotional (13.7\%) and Conditional (5.3\%).
This is consistent with the broader pre-purchase nature of the sessions in
B2V, where instrumental concerns such as price--value, functional efficacy
and perceived risk drive most decisions, while Epistemic and Social drivers
appear less frequently but more selectively.

Different value dimensions also impose different descriptive demands on the
behavior summary itself. Figure~\ref{fig:len-by-tcv} compares the per-sample
summary length across dimensions: emotional decisions yield the longest
summaries (median 770 characters), while Social and Epistemic decisions
require visibly less text. This suggests that aesthetic and affective drivers
manifest through longer chains of observable behavior, whereas social drivers
tend to be expressed more compactly.
...
\subcaption{TCV dimension composition.}
\label{fig:tcv-donut}
\end{minipage}\hfill
\begin{minipage}{0.42\linewidth}
\centering
\includegraphics[width=\linewidth]{image/figs/fig10_length_by_tcv.pdf}
\subcaption{Summary length by TCV dimension.}
\label{fig:len-by-tcv}
\end{minipage}
\caption{Distribution of TCV dimensions and the corresponding descriptive
complexity of behavior summaries.}
\label{fig:tcv}
\end{figure}

\subsection{Temporal Coverage}
B2V is collected across 13 monthly snapshots spanning March 2025 to March 2026,
totaling 5{,}440 annotated sessions and 202{,}771 extracted behavior
interactions. Figure~\ref{fig:per-date} shows the per-snapshot session count
together with the normalized composition of TCV dimensions over time.
...
Conditional, Social and Epistemic shares stay within $\pm$3\% of their global
means. This indicates that the dataset captures consistent decision behavior
across a one-year window rather than reflecting a single seasonal pattern.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{image/figs/fig6_per_date_stacked.pdf}
\caption{Temporal coverage of B2V. Top: number of sessions per monthly
snapshot together with the cumulative collection (red line). Bottom:
normalized composition of TCV dimensions over time.}
\label{fig:per-date}
\end{figure}

\subsection{Multi-label and Construct Co-occurrence}
75.2\% of B2V sessions carry at least one secondary value beyond the primary
label, supporting evaluation under both single-label and multi-label
formulations. Figure~\ref{fig:cooccur} reports the conditional co-occurrence
matrix among the 12 most frequent labels, where cell $(i,j)$ is
...
\caption{Conditional co-occurrence among the 12 most frequent labels:
$P(\text{label}_j \mid \text{label}_i)$.}
\label{fig:cooccur}
\end{figure}

\subsection{Split Balance}
We adopt an 80/10/10 stratified split at the session level, keeping the
relative frequencies of all primary labels approximately constant across the
three partitions. Figure~\ref{fig:split-balance} reports the per-label share
within each split for the 12 most frequent labels. The largest deviation
between any split and the corpus average is 2.9 percentage points
(\emph{aesthetic~appeal} in test, 14.7\% vs.\ 11.9\% overall); the
remaining labels agree within $\pm$2\%. Total token-level statistics
(Table~\ref{tab:b2v-stats}) further confirm that average session length,
behavior--interaction count and multi-label rate match closely across
train, validation and test.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{image/figs/fig9_split_balance.pdf}
\caption{Per-label share within each split for the 12 most frequent labels.
The split keeps the long-tailed distribution intact while preserving label
proportions across partitions.}
\label{fig:split-balance}
\end{figure}
